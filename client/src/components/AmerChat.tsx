...
export default function AmerChat() {
  ...
  // Chat button is hidden by default - only shows when admin sends a message
  // (isVisible is set to true in the chat:fromAdmin handler below)
  ...
  useEffect(() => {
    const handleAdminMessage = (data: { message: string; timestamp: string }) => {
      ...
      // Show chat button (first admin message makes it visible)
      setIsVisible(true);
    ...
    return () => {
      if (socket.value) socket.value.off("chat:fromAdmin", handleAdminMessage);
    };
  }, []);
  ...
  if (!isVisible && !hasNewAdminMessage) return null;

  return (
    <>
      {/* Chat Button */}
      <div
        className="fixed left-4 bottom-4 z-50 cursor-pointer animate-fade-in"
